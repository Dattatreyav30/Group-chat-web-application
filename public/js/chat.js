...
window.addEventListener('DOMContentLoaded', async (e) => {
  ...
  if (messages) {
    ...
    newMessages.forEach(async (obj) => {
      ...
      localStorage.setItem('messages', JSON.stringify(messageFromStorage));
    });
  }
  setInterval(async () => {
    ...
    if (messageFromStorage.length > 0) {
      ...
      const response = await axios.get(`http://localhost:7000/message/getAllMessages?lastMessageDate=${lastMessageDate}`, { headers: { 'authorization': token } });
      const newMessages = response.data.messages;
      newMessages.forEach(async (obj) => {
